...
do
	cd OuterFold_${i}; 
	~/caffe/tools/extra/parse_log.py log/caffe.INFO log --log_index ff_OF${i}_${MODEL}_T${TID}; 
	#/opt/quarantine/caffe/930630/src/caffe/tools/extra/parse_log.py log/caffe.INFO log --log_index ff_OF${i}_${MODEL}_T${TID}; 
	cd ..; 
done
...
for (( i=$S_FOLD; i<=$F_FOLD; i++ ))
do 
	cp OuterFold_${i}/log/caffe.INFO.{train,test}_ff_OF${i}_${MODEL}_T${TID} /projects/nikhil/ADNI_prediction/caffe_output/OuterFold${i}; 
done

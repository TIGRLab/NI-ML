...
if __name__ == '__main__':
    ...
    mask = rawfileh.root.mask.read() > 0 # trick to bianarize multilabel masks

    print "Mask vol: {}".format(np.sum(mask))
    ...
    if not os.path.exists(scoresfile):
        ...
        for i, labelfile in enumerate(labelfiles):
            ...
            scores[i] = 2*float(np.sum(rawdata[i,:] * mask))/(np.sum(rawdata[i,:]>0)+np.sum(mask))

	    #keep a list of all candidate volumes before filtering / masking
	    candidate_vols.append(np.sum(rawdata[i,:]))
    ...
    print  
    print 'Mask vol: {}, candidate vol stats: mean= {}, stdev={}'.format(np.sum(mask), np.mean(np.array(candidate_vols)), np.std(np.array(candidate_vols)))
    ...
    if not os.path.exists(datafile):
        ...
        _ = fileh.create_array(fileh.root,'fullvolmask' ,rawfileh.root.fullmask.read() > 0)
        _ = fileh.create_array(fileh.root,'volmask'     ,rawfileh.root.mask.read() > 0)
	...
	print "Adni2 images: ", sum(["subject" in x for x in images_used])
	print "Adni2 %: ", sum(["subject" in x for x in images_used])/float(len(images_used))
    else:

...
label_node = 'label'
model_path = '/projects/francisco/repositories/NI-ML/models/deepnets/caffe/ff/hc/adcn/left/archive/2015-07-20/'
model_file = model_path + '_iter_50000.caffemodel'
...
H0 = np.identity(X.shape[1]) * sensitivity_factor * np.max(mu_0)
H1 = np.identity(X.shape[1]) * sensitivity_factor * np.max(mu_1)
...
outp, outn, DX0 = Sjk(net, mu_0, H0, sensitivity_factor * np.max(mu_0))
s0k = DX0[:, 0] ** 2

outp1, outn1, DX1 = Sjk(net, mu_1, H1, sensitivity_factor * np.max(mu_1))
s1k = DX1[:, 0] ** 2

normed_sen0, normed_sen1 = normed_sen(s0k, s1k)
...
sample_s0k = np.mean(DX0[:, :, 0], axis=0) ** 2

DX1 = sample_Sjk(net, X_fused_1, sensitivity_factor)
sample_s1k = np.mean(DX1[:, :, 0], axis=0) ** 2

normed_sample_sen0, normed_sample_sen1 = normed_sen(sample_s0k, sample_s1k)
...
brx0 = bDX[:, :, 0]
brx1 = bDX[:, :, 0]
bs0k = np.mean(brx0, axis=0) ** 2
bs1k = np.mean(brx1, axis=0) ** 2

normed_bs0, normed_bs1 = normed_sen(bs0k, bs1k)

# FF1 and FF2 activations:
FFDX = sample_binary_FF_Sjk(net, X_fused, ['ff1', 'ff2'])
FF1 = FFDX['ff1']
FF2 = FFDX['ff2']
FF1mu = np.mean(FF1, axis=0).T ** 2
FF2mu = np.mean(FF2, axis=0).T ** 2
...
baseline_mask = mappings.get_node('/l_datamask')[:]
volmask = mappings.get_node('/l_volmask')[:]
mappings.close()

baseline_shape = volmask.shape

# plt.style.use('ggplot')
sl = [(bs0k, 'binary 0'), (bs1k, 'binary 1'),
      (sample_s0k, 'mean of sen 0'), (sample_s1k, 'mean of sen 1'),
      (s0k, 'sen of mean 0'), (s1k, 'sen of mean 1')]

sl_normed = [(normed_bs0, 'binary 0'), (normed_bs1, 'binary 1'),
      (normed_sample_sen0, 'mean of sen 0'), (normed_sample_sen1, 'mean of sen 1'),
      (normed_sen0, 'sen of mean 0'), (normed_sen1, 'sen of mean 1')]

#sla = [(sx0, 'sx0'), (sx1, 'sx1'), (s0k, 's0k'), (s1k, 's1k')]

plot_slices(sl, baseline_shape, baseline_mask)
plot_slices(sl_normed, baseline_shape, baseline_mask)
#
